...
class IconsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create icon" do
    assert_difference("Icon.count") do
      post icons_url, params: { icon: { image: @icon.image, kind: @icon.kind, tags: @icon.tags, title: @icon.title } }
    end
  ...
  test "should update icon" do
    patch icon_url(@icon), params: { icon: { image: @icon.image, kind: @icon.kind, tags: @icon.tags, title: @icon.title } }
